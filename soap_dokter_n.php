...
WHERE        (NOLAB_RS = '$noreg_igd') and FLAG <>'' order by REG_DATE desc, KEL_PEMERIKSAAN,PARAMETER_NAME
";
...
while   ($dhqlab = sqlsrv_fetch_array($hqlab, SQLSRV_FETCH_ASSOC)){     
	...
	$lab0 = $i.'|'.$dhqlab[REG_DATE].'|'.$dhqlab[TARIF_NAME].' '.trim($dhqlab[PARAMETER_NAME]).' : '.    $dhqlab[HASIL].'('.trim($dhqlab[FLAG]).')';
...
<div class="container-fluid">
	...
	<body onload="document.myForm.ku.focus();">
		...
		<form method="POST" name='myForm' action="" enctype="multipart/form-data">
			...
			<table>
				<tr valign="top">
					<td width='10%'><font size="2"><b>History</b></font>
						<table width="100%">
							<tr>
								<td>Rencana Terapi
									<textarea class="form-control" name="am77" cols="100%" onfocus="nextfield ='';" style="min-height:200px;font-size: 8pt" disabled><?php echo $am77;?></textarea>
								</td>
							</tr>
							<tr>
								<td>advis IGD
									<textarea class="form-control" name="" cols="100%" onfocus="nextfield ='';" style="min-height:300px;font-size: 8pt" disabled><?php echo $terapi_igd;?></textarea>
								</td>
							</tr>
<!-- 							<tr>
								<td>Planning & Instruksi
									<textarea class="form-control" name="" cols="100%" onfocus="nextfield ='';" style="min-height:200px;font-size: 8pt" disabled><?php echo $his_cppt;?></textarea>
								</td>
							</tr>
						-->							<tr>
							<td>Hasil lab
								<textarea class="form-control" name="" cols="100%" onfocus="nextfield ='';" style="min-height:200px;font-size: 8pt" disabled><?php echo $lab;?></textarea>
							</td>
						</tr>
						<tr>
							<td>Hasil Radiologi
								<textarea class="form-control" name="" cols="100%" onfocus="nextfield ='';" style="min-height:100px;font-size: 8pt" disabled><?php echo $rad;?></textarea>
							</td>
						</tr>
					</table>
					...
					<div class="row">
						<div class="col-12 text-left">
							<font size="2">
								...
								$qlc="
								SELECT TOP(3) id,kodedokter,noreg,userid,dpjp,subjektif,objektif,assesment,planning,instruksi,sbu,kodeunit,
								...
								FROM ERM_SOAP WHERE norm='$norm' and noreg like '%R%' and kodedokter='$kodedokter'
								";
								...
								while   ($dl = sqlsrv_fetch_array($hlc, SQLSRV_FETCH_ASSOC)){  
									...
									<td>$namadokter<br>$dl[kodeunit] - $dl[sbu]</td>
									<td>
									$hasilassesment
									</td>
									</tr>

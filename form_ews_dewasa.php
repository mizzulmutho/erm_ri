...
$q		= "select * from ERM_RI_ASSESMEN_AWAL where noreg='$noreg' and jenis like '%EWS_DEWASA%'";
...
if(empty($cekreg)){
	$q		= "select * from ERM_FORM where FORM='EWS_DEWASA'";
	$hasil  = sqlsrv_query($conn, $q);			  
	$data2	= sqlsrv_fetch_array($hasil, SQLSRV_FETCH_ASSOC);	
...
if (isset($_POST["simpan"])) {
	...
	if(empty($cekreg)){
		...
		//jika tidak ada insert
		$q  = "update ERM_RI_ASSESMEN_AWAL set isi='$data',userid='$user',tglupdate='$tgl' where noreg='$noreg' where jenis='EWS_DEWASA'";
		$hs = sqlsrv_query($conn,$q);
	}
	...
	echo "
	<script>
	top.location='form_ews_dewasa.php?id=$id|$user';
	</script>
	";

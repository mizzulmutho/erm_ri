...
$qi="SELECT * FROM  ERM_RI_DIAGNOSA_AWAL where noreg='$noreg'";
...
if (isset($_POST["simpan"])) {
    ...
    $qu="SELECT noreg FROM ERM_RI_DIAGNOSA_AWAL where noreg='$noreg'";
    $h1u  = sqlsrv_query($conn, $qu);        
    $d1u  = sqlsrv_fetch_array($h1u, SQLSRV_FETCH_ASSOC); 
    $cekreg = trim($d1u['noreg']);

    if(empty($cekreg)){
        echo $q  = "insert into  ERM_RI_DIAGNOSA_AWAL(noreg) values ('$noreg')";
        $hs = sqlsrv_query($conn,$q);
    }

    $q  = "update ERM_RI_DIAGNOSA_AWAL set

...
$qi="SELECT noreg FROM ERM_RI_RENCANA_RAWAT where noreg='$noreg'";
$hi  = sqlsrv_query($conn, $qi);        
$di  = sqlsrv_fetch_array($hi, SQLSRV_FETCH_ASSOC); 
$regcek = $di['noreg'];

if(empty($regcek)){
	$q  = "insert into ERM_RI_RENCANA_RAWAT(noreg,userid,tglentry,tgl) values ('$noreg','$user','$tgl','$tgl')";
	$hs = sqlsrv_query($conn,$q);
}else{

	$qe="
	SELECT *,CONVERT(VARCHAR, tgl, 23) as tgl
	FROM ERM_RI_RENCANA_RAWAT
	where noreg='$noreg'";
...
if (isset($_POST["simpan"])) {
	...
	$q  = "update ERM_RI_RENCANA_RAWAT set
	rencana	='$rencana'
	where noreg='$noreg'
	";

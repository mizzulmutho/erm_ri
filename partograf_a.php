		...
		<body onload="document.myForm.pasien_mcu.focus();">
			<font size='2px'>
				<form method="POST" name='myForm' action="" enctype="multipart/form-data">
					...
					<a href='partograf_print.php?id=<?php echo $id.'|'.$user; ?>' class='btn btn-info' target='_blank'><i class="bi bi-printer-fill"></i></a>
				...
				<tr>
					<td>
						<div class="col-12">
							Upload Gambar 1 : 
							<?php 
							$q		= "select par3 from ERM_RI_PARTOGRAF where noreg = '$noreg'";
							$h  = sqlsrv_query($conn, $q);			  
							$d  = sqlsrv_fetch_array($h, SQLSRV_FETCH_ASSOC); 
							$par3 = trim($d['par3']);
							echo "Gambar terupload : <a href='$par3' target='_blank'>$par3</a>";
							?>
							<input type="file" name="par3" class="" >Ekstensi .jpeg | .jpg
						</div>
					</td>
				</tr>
				...
				<tr>
					<td>
						<div class="col-12">
							Upload Gambar 2 : 
							<?php 
							$q		= "select par4 from ERM_RI_PARTOGRAF where noreg = '$noreg'";
							$h  = sqlsrv_query($conn, $q);			  
							$d  = sqlsrv_fetch_array($h, SQLSRV_FETCH_ASSOC); 
							$par4 = trim($d['par4']);
							echo "Gambar terupload : <a href='$par4' target='_blank'>$par4</a>";
							?>
							<input type="file" name="par4" class="" >Ekstensi .jpeg | .jpg
						</div>
					</td>
				</tr>
				...
				<tr>
					<td>
						<div class="col-12">
							Upload Gambar 3 : 
							<?php 
							$q		= "select par5 from ERM_RI_PARTOGRAF where noreg = '$noreg'";
							$h  = sqlsrv_query($conn, $q);			  
							$d  = sqlsrv_fetch_array($h, SQLSRV_FETCH_ASSOC); 
							$par5 = trim($d['par5']);
							echo "Gambar terupload : <a href='$par5' target='_blank'>$par5</a>";
							?>
							<input type="file" name="par5" class="" >Ekstensi .jpeg | .jpg
						</div>
					</td>
				</tr>
...
if (isset($_POST["simpan"])) {
	...
	$par1	= $_POST["par1"];
	$par2	= $_POST["par2"];
	$par3 = $_FILES['par3']['name'];
	$par4 = $_FILES['par4']['name'];
	$par5 = $_FILES['par5']['name'];
	...
	if ($par3) {
		...
		$q  = "update ERM_RI_PARTOGRAF set par3='$path' where noreg='$noreg'";         
		$hs = sqlsrv_query($conn,$q);
	...
	if ($par4) {
		...
		$q  = "update ERM_RI_PARTOGRAF set par4='$path' where noreg='$noreg'";         
		$hs = sqlsrv_query($conn,$q);
		...
		if($hs){
			if($par4){
				unlink($par4a);
			// move_uploaded_file($nama_file, $path);
				move_uploaded_file($_FILES['par4']['tmp_name'],$path);
			}
	...
	if ($par5) {
		...
		$q  = "update ERM_RI_PARTOGRAF set par5='$path' where noreg='$noreg'";         
		$hs = sqlsrv_query($conn,$q);
		...
		if($hs){
			if($par3){
				unlink($par3a);
			// move_uploaded_file($nama_file, $path);
				move_uploaded_file($_FILES['par5']['tmp_name'],$path);
			}
